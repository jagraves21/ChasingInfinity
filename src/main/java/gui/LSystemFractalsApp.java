...
public class LSystemFractals {
	public static final int WIDTH = 800;
	public static final int HEIGHT = 600;

	private static int getFPS(AbstractLSystemFractal fractal) {
		try {
	...
	private static int getUPS(AbstractLSystemFractal fractal) {
		try {
	...
	protected static JComponent getContentPane(
		...
		JComboBox<Supplier<AbstractLSystemFractal>> comboBox
	) {
		// reference to the currently instantiated fractal
		// this is needed for the resetCheck functionality
		final AbstractLSystemFractal[] currentFractal = new AbstractLSystemFractal[1];
		...
		comboBox.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				Supplier<AbstractLSystemFractal> selectedSupplier =
					(Supplier<AbstractLSystemFractal>) comboBox.getSelectedItem();
				...
				new SwingWorker<Void, Void>() {
					...
					protected void done() {
						AbstractLSystemFractal fractal = selectedSupplier.get();
	...
	protected static JComboBox<Supplier<AbstractLSystemFractal>> getComboBox(
		List<Supplier<AbstractLSystemFractal>> fractalSuppliers
	) {
		JComboBox<Supplier<AbstractLSystemFractal>> comboBox = new JComboBox<>();
		for (Supplier<AbstractLSystemFractal> supplier : fractalSuppliers) {
			final String supplierName = getSupplierName(supplier);
			comboBox.addItem(new Supplier<AbstractLSystemFractal>() {
				public AbstractLSystemFractal get() {
					return supplier.get();
				}
	...
	protected static void createAndShowGUI(List<Supplier<AbstractLSystemFractal>> fractalSuppliers) {
		FractalPanel fractalPanel = new FractalPanel();
		JComboBox<Supplier<AbstractLSystemFractal>> comboBox = getComboBox(fractalSuppliers);
	...
	public static void main(String[] args) {
		List<Supplier<AbstractLSystemFractal>> fractalSuppliers = new ArrayList<>();
		LSystemFractalRegistry.loadAllFractals();
		fractalSuppliers.addAll(LSystemFractalRegistry.getAll().values());
